...
void add_hidden_layer(NeuralNet *ann, int layer_size) {
    Layer *new_l = (Layer*) malloc(sizeof(Layer));
    ann->input->dim.w = layer_size;
    new_l->dim.h = layer_size;
    new_l->dim.w = ann->input->next->dim.h;
    new_l->in = allocate_float_1d(new_l->dim.w);
    new_l->out = allocate_float_1d(new_l->dim.w);
    fill_zero(new_l->in, new_l->dim.w);
    fill_zero(new_l->out, new_l->dim.w);
    ...
    new_l->weights = allocate_float_2d(new_l->dim.h, new_l->dim.w);
    ann->input->weights = allocate_float_2d(ann->input->dim.h, ann->input->dim.w);
    ...
    init_weight_matrix(new_l->weights, new_l->dim);
    fill_zero(ann->input->in, ann->input->dim.w);
    fill_zero(ann->input->out, ann->input->dim.w);

    new_l->next = ann->input->next;
    new_l->prev = ann->input;
    ann->input->next = new_l;
    ann->input->next->prev = new_l;
}
