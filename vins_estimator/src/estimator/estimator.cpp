...
void Estimator::optimization()
{
    ...
    // cout << "window index: " << window_index << std::endl;
    // ofstream foutC("/home/frankxcr/VINS-Fusion_ws/res.csv", ios::app);
    // foutC << "1001 " << window_index << endl;
    // foutC.close();
    int new_points = 0;
    int valid_points = 0;
    int predict_points = 0;
    int short_track_points = 0;
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        // cout << "Feature index: " << feature_index << std::endl;
        // ofstream foutC("/home/frankxcr/VINS-Fusion_ws/res.csv", ios::app);
        // foutC << "Feature index: " << feature_index << endl;
        // foutC.close();

        // cout << "start frame: " << it_per_id.start_frame << std::endl;
        // cout << "end frame: " << it_per_id.endFrame() << std::endl;
        // ofstream foutC("/home/frankxcr/VINS-Fusion_ws/res.csv", ios::app);
        // foutC.setf(ios::fixed, ios::floatfield);
        // foutC << residual.transpose() << endl;
        // foutC.close();
        ...
        if(WEIGHT_TD && td_flag)
        {
            ...
            if(it_per_id.feature_per_frame[0].velocity.norm() == 0)
            {
                ...
                // if((k2 - k1).norm() < 0.05)
                //     it_per_id.feature_per_frame[0].velocity = it_per_id.feature_per_frame[1].velocity - k1;
                
                
                // cout << "tracking length: "  << it_per_id.endFrame() - it_per_id.start_frame  << std::endl;
                vector<double *> trainSet;
                vector<double *> labelSet;
                if(it_per_id.endFrame() - it_per_id.start_frame > 3)
                {
                    cout << "predict_track_points" << ++short_track_points << std::endl;
                    double *input=(double*)malloc(sizeof(double)*2);
                    double *label=(double*)malloc(sizeof(double)*2);
                    double vx_max = -100;
                    double vx_min = 100;
                    double vy_max = -100;
                    double vy_min = 100;

                    for(int i = 1; i <  it_per_id.endFrame() - it_per_id.start_frame - 1; i++)
                    {
                        double vx = it_per_id.feature_per_frame[it_per_id.endFrame() - it_per_id.start_frame - i].velocity.x();
                        double vy = it_per_id.feature_per_frame[it_per_id.endFrame() - it_per_id.start_frame - i].velocity.y();
                        if(vx_max < vx)
                            vx_max = vx;
                        if(vx_min > vx)
                            vx_min = vx;
                        ...
                        if(vy_min > vy)
                            vy_min = vy;
                        // cout << "velocity " << "i: " <<  it_per_id.feature_per_frame[i].velocity.transpose() << std::endl;
                    }

                    for(int i = 1; i <  it_per_id.endFrame() - it_per_id.start_frame - 1; i++)
                    {
                        input[0] = (it_per_id.feature_per_frame[it_per_id.endFrame() - it_per_id.start_frame - i].velocity.x() - vx_min) / (vx_max - vx_min);
                        input[1] = (it_per_id.feature_per_frame[it_per_id.endFrame() - it_per_id.start_frame - i].velocity.y() - vy_min) / (vy_max - vy_min);
                        // input[2] = (it_per_id.endFrame() - it_per_id.start_frame - i) * 0.005;
                        label[0] = (it_per_id.feature_per_frame[it_per_id.endFrame() - it_per_id.start_frame - i - 1].velocity.x() - vx_min) / (vx_max - vx_min);
                        label[1] = (it_per_id.feature_per_frame[it_per_id.endFrame() - it_per_id.start_frame - i - 1].velocity.y() - vy_min) / (vy_max - vy_min);
                    ...
                    for(int i = 1; i <  it_per_id.endFrame() - it_per_id.start_frame; i++)
                    {
                        ...
                        test[0] = (it_per_id.feature_per_frame[it_per_id.endFrame() - it_per_id.start_frame - i].velocity.x() - vx_min) / (vx_max - vx_min);
                        test[1] = (it_per_id.feature_per_frame[it_per_id.endFrame() - it_per_id.start_frame - i].velocity.y() - vy_min) / (vy_max - vy_min);
                        // test[2] = (it_per_id.endFrame() - it_per_id.start_frame - i) * 0.005;
                        label[0] = (it_per_id.feature_per_frame[it_per_id.endFrame() - it_per_id.start_frame - i - 1].velocity.x() - vx_min) / (vx_max - vx_min);
                        label[1] = (it_per_id.feature_per_frame[it_per_id.endFrame() - it_per_id.start_frame - i - 1].velocity.y() - vy_min) / (vy_max - vy_min);
                        double *z=lstm->predict(test);
                        // cout << "predict: " <<  z[0] << " " << z[1] << "real" << label[0] << " " << label[1]  << std::endl;
                        ...
                        free(z);
                        free(test);
                        // if(new_points - predict_points > 15)
                        //     problem.SetParameterBlockConstant(para_Td[0]);
                    }
                    // cout << "##############" << std::endl;
                    // // lstm->~Lstm();
                ...
                else
                {
                    ...
                    //     // cout << "predict: " <<  z[0] << " " << z[1] << "real" << label[0] << " " << label[1]  << std::endl;
                    //     it_per_id.feature_per_frame[it_per_id.endFrame() - it_per_id.start_frame - i - 1].velocity = Vector2d(10.0,10.0);
                    // }
                }
                // FOR(i, trainSet.size()){
                //     free(trainSet[i]);
            ...
            // {
                if(valid_3d_points[it_per_id.start_frame].size() > 35 && new_flag)
                {
                    ...
                    float **X_test = allocate_float_2d(test_dim.h, test_dim.w);
                    float **y_train = allocate_float_2d(train_dim.h, train_dim.w);
                    float *J, *acc;
                    ...
                    X_train_min = allocate_float_1d(3);
                    X_train_max = allocate_float_1d(3);
                    y_train_min = allocate_float_1d(2);
                    y_train_max = allocate_float_1d(2);


                    minmax_scaler(X_train,train_dim, X_train_min, X_train_max);
                    // cout << "##############################" << endl;
                    ...
                    for(int k = 0; k < test_dim.h; ++k)
                    {
                        ...
                        X_test[k][2] =  (X_test[k][3] - X_train_min[2]) / (X_train_max[2] - X_train_min[2]);
                        feed_forward_net(ann, X_test[k]);
                        cout << ann->output->out[0] * (y_train_max[0] - y_train_min[0]) + y_train_min[0] << " " << ann->output->out[1] * (y_train_max[1] - y_train_min[1]) + y_train_min[1] << endl;
                        it_per_id.feature_per_frame[0].velocity = Vector2d(ann->output->out[0] * (y_train_max[0] - y_train_min[0]) + y_train_min[0], ann->output->out[1] * (y_train_max[1] - y_train_min[1]) + y_train_min[1]);
                        it_per_id.feature_per_frame[0].velocityRight = Vector2d(ann->output->out[0] * (y_train_max[0] - y_train_min[0]) + y_train_min[0], ann->output->out[1] * (y_train_max[1] - y_train_min[1]) + y_train_min[1]);
                        new_flag = 0;
                    ...
                    free_float_1d(J);
                    free_float_1d(X_train_max);
                    free_float_1d(X_train_min);
                    free_float_1d(y_train_max);
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            if (imu_i != imu_j)
            {
                ...
                // cout << "index: "  << imu_j << " " << it_per_frame.velocity.transpose() << std::endl;
            }
            ...
            {                
                ...
                if(imu_i != imu_j)
                {
                    ...
                    // Eigen::Vector3d tic2(para_Ex_Pose[1][0], para_Ex_Pose[1][1], para_Ex_Pose[1][2]);
                    // Eigen::Quaterniond qic2(para_Ex_Pose[1][6], para_Ex_Pose[1][3], para_Ex_Pose[1][4], para_Ex_Pose[1][5]);
                    ...
                    // velocity_i.z() = 0;
                    // velocity_j.x() = it_per_frame.velocityRight.x();
                    // velocity_j.y() = it_per_frame.velocityRight.y();
                    // velocity_j.z() = 0;

                    // Eigen::Vector3d pts_i_td, pts_j_td;
                    // pts_i_td = pts_i - (td - td_i) * velocity_i;
                    // pts_j_td = pts_j_right - (td - td_j) * velocity_j;
                    ...
                    // residual = ProjectionTwoFrameTwoCamFactor::sqrt_info * residual;
                    // cout << "residual: " << residual.transpose() << std::endl;
                    // ofstream foutC("/home/frankxcr/VINS-Fusion_ws/res.csv", ios::app);
                    // foutC.setf(ios::fixed, ios::floatfield);
                    // foutC << residual.transpose() << " " << window_index + it_per_id.start_frame + imu_j - imu_i << endl;
                    // foutC.close();
                }
                else
                {
